...
```{r initialisation, echo=TRUE, message=FALSE}
library(data.table)
library(dplyr)
library(ggplot2)
library(grid)
...
* there are `r n_obs` intervals recorded over 61 days
* there are 288 intervals in a single day
* Number of unique days is `r days_u`
...
```{r steps per day, warning=FALSE, fig.height=4}
...
* The mean total number of steps per day is `r d_mean`
* The median total number of steps per day is `r d_median`
...
```{r single day, comment="", fig.height=3}
...
```{r total steps comparison, warning=FALSE}
...
```{r weekly activity, echo=FALSE}
activity_i <- mutate(activity_i, 
       day_type = ifelse(weekdays(date) %in% c("Saturday", "Sunday"),
                         "weekend", "weekday") )  %>% 
    as.data.table()

act_by_interval_end <- activity_i[day_type == "weekend", 
                                .(mean(steps), day_type), 
                                by = interval] %>% 
    setnames("V1", "avg_steps") %>% 
    unique()

act_by_interval_day <- activity_i[day_type == "weekday", 
                                .(mean(steps), day_type), 
                                by = interval] %>% 
    setnames("V1", "avg_steps") %>% 
    unique()
